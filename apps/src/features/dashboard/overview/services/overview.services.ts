...
export async function getOverviewStats() {
    try {
        const data = await fetcherWithAuth(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/overview/stats`, {
            method: 'GET',
            cache: 'no-store',
        });
...
export async function getTopDeals() {
    try {
        const data = await fetcherWithAuth(`${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/overview/top-deals`, {
            method: 'GET',
            cache: 'no-store',
        });
